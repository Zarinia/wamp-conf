...
function check_virtualhost($check_files_only = false) {
	...
	$virtualHost['vhosts_file'] = $c_apacheVersionDir.'/apache'.$wampConf['apacheVersion'].'/'.$wampConf['apacheConfDir'].'/extra/httpd-vhosts.conf';
	...
	$virtualHost['documentPath'] = array_merge($Document_matches[1]);
	for($i = 0 ; $i < $nb_Document ; $i++) {
		$chemin = $Document_matches[1][$i];
