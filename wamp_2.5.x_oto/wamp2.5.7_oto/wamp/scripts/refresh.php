...
function update_wampmanager_tpl($item_on, $test_value, $equal_to_on, $equal_to_off) {
	global $wampTplFileContents, $replace_tpl;
	$item_off = ";".$item_on;
	$item_is_off = $item_is_on = false;
	if(strpos($wampTplFileContents, $item_off) !== false)
		$item_is_off = true;
	elseif(strpos($wampTplFileContents, $item_on) !== false)
		$item_is_on = true;
	if($item_is_on && $test_value == $equal_to_off) {
		$wampTplFileContents = str_replace($item_on, $item_off, $wampTplFileContents);
		$replace_tpl = true;
	}
	if($item_is_off && $test_value == $equal_to_on) {
		$wampTplFileContents = str_replace($item_off, $item_on, $wampTplFileContents);
		$replace_tpl = true;
	}
}
...
if(!empty($wampConf['HomepageAtStartup']))
	update_wampmanager_tpl("Action: run; FileName: \"\${c_navigator}\"; Parameters: \"http://localhost\${UrlPort}/\"; ShowCmd: normal; Flags: ignoreerrors",$wampConf['HomepageAtStartup'], "on", "off");
// Item menu Online / Offline
if(!empty($wampConf['MenuItemOnline']))
	update_wampmanager_tpl("Type: item; Caption: \"\${w_putOnline}\"; Action: multi; Actions: onlineoffline",
	$wampConf['MenuItemOnline'], "on", "off");
// Item submenu Apache Service Check port used (if not 80)
if(!empty($wampConf['apacheUseOtherPort']))
	update_wampmanager_tpl("Type: item; Caption: \"\${w_testPortUsed}", $wampConf['apacheUseOtherPort'], "on", "off");
...
';
	  elseif ($params_for_wampconf[$paramname] == -1) 
	  	$wampConfText .= '['.$wampConfParams[$paramname].']
Action: service; Service: '.$c_apacheService.'; ServiceAction: stop; Flags: waituntilterminated
Action: run; FileName: "'.$c_phpCli.'";Parameters: "switchWampParam.php '.$wampConfParams[$paramname].' create off [php] [main]";WorkingDir: "'.$c_installDir.'/scripts"; Flags: waituntilterminated
Action: run; FileName: "'.$c_phpCli.'";Parameters: "-c . refresh.php";WorkingDir: "'.$c_installDir.'/scripts"; Flags: waituntilterminated
Action: run; FileName: "net"; Parameters: "start '.$c_apacheService.'"; ShowCmd: hidden; Flags: waituntilterminated
Action: resetservices
Action: readconfig;
