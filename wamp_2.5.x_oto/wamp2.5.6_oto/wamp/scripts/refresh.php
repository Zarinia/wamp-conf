...
if($c_UsedPort == $c_DefaultPort) {
	$w_AlternatePort = sprintf($w_UseAlternatePort, $c_AlternatePort, $c_UsedPort);
	$c_PortToUse = $c_AlternatePort;
	$UrlPort = '';
}
else {
	$w_AlternatePort = sprintf($w_UseAlternatePort, $c_DefaultPort, $c_UsedPort);
	$c_PortToUse = $c_DefaultPort;
	$UrlPort = ':'.$c_UsedPort;
}
// ************************
...
$startup_line_on = "Action: run; FileName: \"\${c_navigator}\"; Parameters: \"http://localhost".$UrlPort."/\"; Flags: ignoreerrors";
...
// Item submenu Apache Service Check port used (if not 80)
$item_port_used_on = "Type: item; Caption: \"\${w_testPortUsed}";
$item_port_used_off = ";Type: item; Caption: \"\${w_testPortUsed}";
//Check if item need to be activated
$test_used = $test_not_used = $replace_tpl = false;
if(strpos($wampTplFileContents, $item_port_used_off) !== false)
	$test_not_used = true;
elseif(strpos($wampTplFileContents, $item_port_used_on) !== false)
	$test_used = true;
if($test_used || $test_not_used) {
	if($c_UsedPort == $c_DefaultPort && $test_used) { //Item to be desactivated
		$wampTplFileContents = str_replace($item_port_used_on, $item_port_used_off, $wampTplFileContents);
		$replace_tpl = true;
	}
	elseif($c_UsedPort != $c_DefaultPort && $test_not_used) { //Item to be activated
		$wampTplFileContents = str_replace($item_port_used_off, $item_port_used_on, $wampTplFileContents);
		$replace_tpl = true;
	}
	if($replace_tpl)
		file_put_contents($templateFile, $wampTplFileContents);
...
";
			...
			$nb_Virtual = preg_match_all("/^(?![\s\t]*#).*<VirtualHost \*:(.*)>$/m", $myVhostsContents, $Virtual_matches);
...
';    	
				...
				//Check number of <VirtualHost equals to number of ServerName
...
';    	
				...
				if(count($Virtual_matches[1]) != $nb_Virtual) {
