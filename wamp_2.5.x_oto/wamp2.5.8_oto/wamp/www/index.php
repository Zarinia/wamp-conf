...
// Ajout du port dans les urls si différent de 80
...
//[modif oto] on récupère la valeur de apachePortUsed
$port = '80';
if(preg_match('|apachePortUsed = "(.*)"|',$wampConfFileContents,$result) !== 0)
	$port = $result[1];
if($port == '80')
	$UrlPort = '';
else
...
if($VirtualHostMenu == "on") {
	...
	if(in_array("#Include conf/extra/httpd-vhosts.conf", $myhttpd) !== false
		...
		if(is_file($vhostConfFile)) {
			...
			$nb_Directory = preg_match_all("/^(?![ \t]*#).*<Directory.*$/m", $myVhostsContents, $Dir_matches);
			if($nb_Server > 0) {
